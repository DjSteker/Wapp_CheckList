...
Public Class Form_ClienteTCP
    ...
    Friend Sub ActulizarListaServodores()
        Try
            ...
            DataGridView_ListaServidores.Rows.Clear()
    ...
    Private Sub Button_GuardarServer_Click(sender As Object, e As EventArgs) Handles Button_GuardarServer.Click
        Try
            ...
            Colunas.NombreServer = TextBox_NombreServer.Text
...
#Region "Voz"
    ...
    Dim DICCIONARIO_IP As New SortedDictionary(Of String, String)
    Dim DICCIONARIO_PUERTO As New SortedDictionary(Of String, String)
    ...
    Private Sub Button_VozAgregar_Click(sender As Object, e As EventArgs) Handles Button_VozAgregar.Click
        Try
            ...
            ENUMERADOR = DICCIONARIO_IP.GetEnumerator
            ...
            If Existe = False Then
                ...
                My.Computer.FileSystem.WriteAllText(Application.StartupPath & "\BaseDatos\" & "CONTACTOS.txt", "$" & CONTACTO & "$" & IP & vbCrLf, True)
        ...
        Try
            ...
            If Existe = False Then
                ...
                CONTACTO = TextBox_Nombre.Text
                PUERTO = TextBox_VozPuerto.Text
                My.Computer.FileSystem.WriteAllText(Application.StartupPath & "\BaseDatos\CONTACTOS_puerto.txt", "$" & CONTACTO & "$" & PUERTO & vbCrLf, True)
                '  MsgBox("SE HA CREADO EL USUARIO: " & CONTACTO & "  IP: " & IP)
                ACTUALIZAR_CONTACTOS()
                TextBox_Nombre.Text = ""
                TextBox_VozIp.Text = ""
                TextBox_VozPuerto.Text = ""
            Else
                MsgBox("Ya existe un contacto con el mismo nombre.")
            End If
        Catch ex As Exception
    ...
    Private Sub ListBox_Contactos_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ListBox_Contactos.SelectedIndexChanged
        ...
        End Try
        Try
            Dim ENUMERADOR2 As IDictionaryEnumerator
            ENUMERADOR2 = DICCIONARIO_PUERTO.GetEnumerator
            While ENUMERADOR2.MoveNext
                If ENUMERADOR2.Key = ListBox_Contactos.SelectedItem Then
                    TextBox_VozPuerto.Text = ENUMERADOR2.Value
                    TextBox_Nombre.Text = ENUMERADOR2.Key
                End If
            End While
        Catch ex As Exception
    ...
    Public Sub ACTUALIZAR_CONTACTOS()
        Try
            ...
            DICCIONARIO_IP.Clear()

            ' RECORREMOS EL FICHERO CONTACTOS.txt PARA LLENAR EL LISTBOX
            Dim filename As String = Application.StartupPath & "\BaseDatos\" & "CONTACTOS.txt"
            ...
            Using parser As New Microsoft.VisualBasic.FileIO.TextFieldParser(filename)
                ...
                While Not parser.EndOfData
                    ...
                    DICCIONARIO_IP.Add(fields(1), fields(2))
                End While
            End Using
        ...
        Try
            ...
            DICCIONARIO_PUERTO.Clear()

            ' RECORREMOS EL FICHERO CONTACTOS.txt PARA LLENAR EL LISTBOX
            Dim filename As String = Application.StartupPath & "\BaseDatos\" & "CONTACTOS_puerto.txt"
            Dim fields As String()
            Dim delimiter As String = "$"
            Using parser As New Microsoft.VisualBasic.FileIO.TextFieldParser(filename)
                parser.SetDelimiters(delimiter)
                While Not parser.EndOfData
                    ...
                    ' CREAMOS UN ARRAY DE TIPO DICCIONARIO CON LOS VALORES QUE OBTENEMOS AL RECORRER EL ARCHIVO DE DATOS
                    DICCIONARIO_PUERTO.Add(fields(1), fields(2))
                End While
            End Using
            'ORDENAMOS ALFABETICAMENTE EL LISTBOX
            ListBox_Contactos.Sorted = True
...
'Dim returndata As String = String.Empty
'serverStream = clientSocket.GetStream()
'Dim inStream(TamañoBuffer) As Byte
'serverStream.Read(inStream, 0, inStream.Length)
'returndata &= Module_SocketTcp.Comvertir_ByteToString(inStream)
'Dim bbbb As String = Module_SocketTcp.Comvertir_ByteToString(inStream)
'Dim aaasa As Long = returndata.Length
...
'Private Sub Button_Conectar_Click()
'Try
'If Button_Conectar.Text = "Conectar" Then
'Conectado = True
'clientSocket = New System.Net.Sockets.TcpClient()
...
'clientSocket.Connect(lipa, TextBox_ConectarDestino_Puerto.Text)
'serverStream = clientSocket.GetStream()
'Dim outStream As Byte() = Module_SocketTcp.Comvertir_StringToByte(Module_SocketTcp.NET_Cavecera & Module_SocketTcp.NET_NuevaConexion & NET_FinCadena)
'serverStream.Write(outStream, 0, outStream.Length)
'serverStream.Flush()
'ctThread = New Threading.Thread(AddressOf getMessage)
'ctThread.Start()
